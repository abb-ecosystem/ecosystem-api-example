...
(function (o) {
  if (!o.hasOwnProperty('Selector_A')) {
    ...
    o.Selector_A = class Selector extends TComponents.Component_A {
      constructor(container, selected = '', label = '', itemList = ['']) {
      ...
      mapComponents() {
        return {
          ddMenu: new TComponents.Dropdown_A(this.find('.dd-menu'), this._itemList, this._selected),
        };
      }
    ...
    o.SelectorVariables_A = class SelectorVariables extends TComponents.Selector_A {
      constructor(
        ...
        super(container, selected, label);
        this.module = module;
      ...
      async onInit() {
        if (this.module) {
          try {
            ...
            const vars = await this.task.searchVariables(this.module, this.isInUse);
       ...
       */
      async updateSearch(module, render = true) {
        this.module = module;
        await this.init();
      }
    };
    ...
    o.SelectorProcedures_A = class SelectorProcedures extends TComponents.Selector_A {
      ...
      async onInit() {
        if (this.module) {
          try {
            if (!this.task) this.task = await API.RAPID.getTask();
            const procs = await this.task.searchProcedures(this.module, this.isInUse);
            this._updateItemList(
              procs.map((p) => p.name),
              this._addNoSelection
            );
    ...
    o.SelectorModules_A = class SelectorModules extends TComponents.Selector_A {
      constructor(container, isInUse = false, selected = '', label = '', addNoSelection = true) {
        super(container, selected, label);
        this.isInUse = isInUse;
        this._addNoSelection = addNoSelection;
      }
      ...
      async onInit() {
        if (!this.task) this.task = await API.RAPID.getTask();
        const modules = await this.task.searchModules(this.isInUse);
        this._updateItemList(
    ...
    o.SelectorEthernetIPDevices_A = class SelectorEthernetIPDevices extends TComponents.Selector_A {
      constructor(container, selected = '', label = '', addNoSelection = true) {
        super(container, selected, label);
        this._addNoSelection = addNoSelection;
      }
    ...
    o.SelectorSignals_A = class SelectorSignals extends TComponents.Selector_A {
      constructor(container, filter = {}, selected = '', label = '', addNoSelection = true) {
        super(container, selected, label);
        this._filter = filter;
        this._addNoSelection = addNoSelection;
      }
