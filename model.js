...
model = {
  ...
  updateSignalAttributes: async function () {
    this.state.ios.inputs.forEach(async (signal) => {
      await signal.updateSignalAttributes()
    })
    this.state.ios.outputs.forEach(async (signal) => {
      await signal.updateSignalAttributes()
    })
  },

  isEverySignalFound: function () {
    let found = this.state.ios.inputs.every((signal) => signal.found === true)
    found =
      found && this.state.ios.outputs.every((signal) => signal.found === true)

    return found
  },

  isEverySignalConfigured: async function () {
    let isCfg = this.state.ios.inputs.every(async (signal) => {
      const is = await Signal.isSignalConfig(signal.name)
      return is
    })
    isCfg =
      isCfg &&
      this.state.ios.outputs.every(async (signal) => {
        const is = await Signal.isSignalConfig(signal.name)
        return is
      })
  ...
  isAnyInputMappedTo: function (attr) {
    ...
    const device = this.state.eth.data.find((dev) => dev.device === attr.Device)
    const found =
      device &&
      device.signals.some(
        (signal) =>
