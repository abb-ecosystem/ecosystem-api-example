  ...
  (function (o) {
    ...
    o.Rapid = new (function () {
      ...
      function Monitor(resource, task = '') {
        ...
        this.onchanged = async function (newValue) {
          ...
          switch (resourceName) {
            ...
            case 'motion-pointer':
              let pp = {};
              pp['moduleName'] = newValue.hasOwnProperty('module-name')
                ? newValue['module-name']
                : '';
              pp['routineName'] = newValue.hasOwnProperty('routine-name')
                ? newValue['routine-name']
                : '';
              pp['beginPosition'] = newValue.hasOwnProperty('BegPosLine')
                ? newValue.BegPosLine
                : '';
        ...
        const processUIInstr = async (uiinstr) => {
          if (
            uiinstr.hasOwnProperty('instr') === false ||
            uiinstr.hasOwnProperty('event') === false
          ) {
          ...
          switch (uiinstr['event']) {
            ...
            default:
              o.writeDebug(
                `Unsupported uiinstr event '${uiinstr['event']}' for instruction '${uiinstr['instr']}'`
              );
              return;
          }
          return data;
        ...
        const getUIInstrData = (item) => {
          if (item.hasOwnProperty('_type') === false || item._type !== 'rap-uiparams-li')
            return null;
          ...
          const getValue = async () => {
            ...
            switch (resourceName) {
              ...
              case 'motion-pointer':
                ...
                break;
              case 'uiinstr':
                if (rawValue.hasOwnProperty('state'))
                  parsedValue = await processUIInstr(rawValue.state[0]);
                break;
              default:
      ...
      function Task(name) {
         ...
         * @returns {Promise<status>}                   A Promise with the status of the call.
         */
        this.movePPToRoutine = function (routineName, userLevel = false) {
          if (typeof routineName !== 'string')
            return Promise.reject("Parameter 'routineName' is not a string.");
          if (typeof userLevel !== 'boolean')
            return Promise.reject("Parameter 'userLevel' is not a boolean.");

          let url = `/rw/rapid/tasks/${encodeURIComponent(
            taskName
          )}/pcp/routine?mastership=implicit`;
          let body = `routine=${encodeURIComponent(routineName)}&userlevel=${userLevel}`;
      ...
      function Module(task, module) {
         ...
         */
        var refreshProperties = function () {
          let url = `/rw/rapid/tasks/${encodeURIComponent(taskName)}/modules/${encodeURIComponent(
            moduleName
          )}`;
      ...
      function Data(task, module, symbol) {
        ...
        var symbolTitle =
          isSharedModule === true
            ? `RAPID/${symbolName}`
            : `RAPID/${taskName}/${moduleName}/${symbolName}`;
        ...
        this.getArrayItem = async function (...indexes) {
          ...
          if (indexes.length > dimensions.length)
            return rejectWithStatus(
              'Get array item failed. >>> More indexes provided than array contains dimensions.'
            );
          for (let iii = 0; iii < indexes.length; iii++) {
            if (
              typeof indexes[iii] !== 'number' ||
              dimensions[iii] < indexes[iii] ||
              indexes[iii] < 1
            )
         ...
         */
        this.setValue = (value) => {
          if (hasValidSetup === false)
            return Promise.reject(`Symbol path '${getSymbolUrl()}' not valid.`);
        ...
        this.setRawValue = (value, ...indexes) => {
          if (hasValidSetup === false)
            return rejectWithStatus(`Symbol path '${getSymbolUrl()}' not valid.`);
        ...
        this.setArrayItem = function (value, ...indexes) {
          ...
          if (indexes.length > dimensions.length)
            return rejectWithStatus(
              'Set array item failed. >>> More indexes provided than array contains dimensions.'
            );
          for (let iii = 0; iii < indexes.length; iii++) {
            if (
              typeof indexes[iii] !== 'number' ||
        ...
        const getParsedType = function (data) {
          if (parsedType !== null && typeof parsedType !== 'undefined')
            return Promise.resolve(parsedType);
          ...
          try {
            ...
            if (rdType.isRecord === false)
              return Promise.reject(
                `Get record indexes failed. >>> Data type '${rdType.url}' is not a Record.`
              );
          ...
          return new Promise((resolve, reject) => {
            ...
            return o.Network.get(url)
              ...
              .catch((err) =>
                reject(
                  toStatus(
                    `Error fetching Rapid data item with index = ${indexes.toString()}.`,
                    err
                  )
                )
              );
              ...
              return o.Network.post(url, body)
                .then(() => {
                  return o.Mastership.release()
                    ...
                    .catch((err) =>
                      Promise.reject('Item value set but failed to release mastership. >>> ' + err)
                    );
                })
                .catch((err) => {
                  let handled = false;
            ...
            .then((res) => {
              ...
              for (const item of obj._embedded.resources) {
                if (
                  ...
                  if (item.hasOwnProperty('local') && item.local.toLowerCase() === 'true')
                    scope = 'local';
                  else if (
        ...
        this.onchanged = function (newValue) {
          this.fetch()
            ...
            .catch((err) =>
              o.writeDebug(`Failed to get value for '${this.getTitle()}'. >>> ${err}`)
            );
        };
        ...
        function getSymbolUrl() {
          ...
          url +=
            isSharedModule === true
              ...
              : `${encodeURIComponent(taskName)}/${encodeURIComponent(
                  moduleName
                )}/${encodeURIComponent(symbolName)}`;
          return url;
        }
      ...
      function parsePointers(obj) {
        ...
        for (const item of obj.state) {
          if (item._type === 'pcp-info' && item._title === 'progpointer') {
            pcp.programPointer['moduleName'] = item.hasOwnProperty('modulemame')
              ? item.modulemame
              : '';
            pcp.programPointer['routineName'] = item.hasOwnProperty('routinename')
              ? item.routinename
              : '';
            pcp.programPointer['beginPosition'] = item.hasOwnProperty('beginposition')
              ? item.beginposition
              : '';
            pcp.programPointer['endPosition'] = item.hasOwnProperty('endposition')
              ? item.endposition
              : '';
          ...
          if (item._type === 'pcp-info' && item._title === 'motionpointer') {
            pcp.motionPointer['moduleName'] = item.hasOwnProperty('modulemame')
              ? item.modulemame
              : '';
            pcp.motionPointer['routineName'] = item.hasOwnProperty('routinename')
              ? item.routinename
              : '';
            pcp.motionPointer['beginPosition'] = item.hasOwnProperty('beginposition')
              ? item.beginposition
              : '';
            pcp.motionPointer['endPosition'] = item.hasOwnProperty('endposition')
              ? item.endposition
              : '';
      ...
      this.startExecution = ({
        ...
        if (validRegainModes.hasOwnProperty(regainMode) === false)
          rejectWithStatus("Illegal parameter 'regainMode'");
        if (validExecutionModes.hasOwnProperty(executionMode) === false)
          rejectWithStatus("Illegal parameter 'executionMode'");
        if (validCycleModes.hasOwnProperty(cycleMode) === false)
          rejectWithStatus("Illegal parameter 'cycleMode'");
        if (validConditions.hasOwnProperty(condition) === false)
          rejectWithStatus("Illegal parameter 'condition'");
      ...
      this.stopExecution = ({ stopMode = 'stop', useTSP = 'normal' } = {}) => {
        ...
        if (validStopModes.hasOwnProperty(stopMode) === false)
          rejectWithStatus("Illegal parameter 'stopMode'");
        if (validUseTSPOptions.hasOwnProperty(useTSP) === false)
          rejectWithStatus("Illegal parameter 'useTSP'");
      ...
      this.searchSymbols = (
        ...
        const checkProperties = (properties) => {
          ...
          if (properties.hasOwnProperty('isInstalled') !== true) {
            ...
            if (typeof properties.isInstalled !== 'boolean')
              errors.push("Search property 'isInstalled' is invalid.");
          }

          if (properties.hasOwnProperty('isShared') !== true) {
            errors.push("Search property 'isShared' is missing.");
          } else {
            if (typeof properties.isShared !== 'boolean')
              errors.push("Search property 'isShared' is invalid.");
          }
          ...
          if (properties.hasOwnProperty('method') !== true) {
            ...
            if (
              properties.method !== this.SearchMethods.block &&
              properties.method !== this.SearchMethods.scope
            )
            ...
            if (properties.hasOwnProperty('searchURL') !== true) {
              ...
              if (typeof properties.searchURL !== 'string')
                errors.push("Search property 'searchURL' is invalid.");
            }
          }
          ...
          if (properties.hasOwnProperty('types') !== true) {
            ...
            if (typeof properties.types !== 'number')
              errors.push("Search property 'types' is invalid.");
          }

          if (properties.hasOwnProperty('recursive') !== true) {
            errors.push("Search property 'recursive' is missing.");
          } else {
            if (typeof properties.recursive !== 'boolean')
              errors.push("Search property 'recursive' is invalid.");
          }

          if (properties.hasOwnProperty('skipShared') !== true) {
            errors.push("Search property 'skipShared' is missing.");
          } else {
            if (typeof properties.skipShared !== 'boolean')
              errors.push("Search property 'skipShared' is invalid.");
          }

          if (properties.hasOwnProperty('isInUse') !== true) {
            errors.push("Search property 'isInUse' is missing.");
          } else {
            if (typeof properties.isInUse !== 'boolean')
              errors.push("Search property 'isInUse' is invalid.");
          }
        ...
        const getBodyText = (properties, dataType, regexp) => {
          let text = ``;
          if ((properties.method & this.SearchMethods.scope) == this.SearchMethods.scope)
            text = `view=scope`;
          else if ((properties.method & this.SearchMethods.block) == this.SearchMethods.block)
            text = `view=block`;

          text += `&blockurl=${encodeURIComponent(properties.searchURL)}`;

          if ((properties.types & this.SymbolTypes.constant) == this.SymbolTypes.constant)
            text += `&symtyp=con`;
          if ((properties.types & this.SymbolTypes.variable) == this.SymbolTypes.variable)
            text += `&symtyp=var`;
          if ((properties.types & this.SymbolTypes.persistent) == this.SymbolTypes.persistent)
            text += `&symtyp=per`;
          if ((properties.types & this.SymbolTypes.function) == this.SymbolTypes.function)
            text += `&symtyp=fun`;
          if ((properties.types & this.SymbolTypes.procedure) == this.SymbolTypes.procedure)
            text += `&symtyp=prc`;
          if ((properties.types & this.SymbolTypes.trap) == this.SymbolTypes.trap)
            text += `&symtyp=trp`;
          if ((properties.types & this.SymbolTypes.module) == this.SymbolTypes.module)
            text += `&symtyp=mod`;
          if ((properties.types & this.SymbolTypes.task) == this.SymbolTypes.task)
            text += `&symtyp=tsk`;
          ...
          if (dataType !== '') {
            if (properties.isShared === true || properties.isInstalled === true) {
              ...
              typurl = typurl.toUpperCase().startsWith('/RAPID')
                ? typurl
                : encodeURIComponent(`RAPID${typurl}`);
              text += `&typurl=${typurl}`;
            }
          }
    ...
    o.IO = new (function () {
      ...
      function Device(network, device) {
        ...
        var devicePath =
          isUnassigned === true
            ? ''
            : `devices/${encodeURIComponent(network)}/${encodeURIComponent(device)}`;
        ...
        this.getSignal = function (signalName) {
          let signal = new RWS.IO.createSignal_internal(networkName, deviceName, signalName);
          return signal
            .fetch()
      ...
      function Signal(network, device, signal) {
        ...
        var signalPath =
          isUnassigned === true
            ...
            : `${encodeURIComponent(network)}/${encodeURIComponent(device)}/${encodeURIComponent(
                signal
              )}`;

        var networkName = isNonEmptyString(network) === true ? network : '';
        var deviceName = isNonEmptyString(device) === true ? device : '';
        var signalName = isNonEmptyString(signal) === true ? signal : '';
        var category = null;
        var signalType = null;
        var signalValue = null;
        var signalWriteAccessLevel = null;
        ...
        /**
         * Gets the write access level of the signal
         *
         * @returns {string}    The write access level
         */
        this.getWriteAccessLevel = function () {
          return signalWriteAccessLevel;
        };
            ...
            .then((res) => {
              ...
              for (const item of obj._embedded.resources) {
                if (item._type === 'ios-signal-li') {
                  ...
                  else signalValue = parseInt(item.lvalue);
        ...
        this.onchanged = function (newValue) {
          ...
          if (this.signalType === 'AI' || this.signalType === 'AO')
            this.signalValue = parseFloat(lvalue);
        ...
        const raiseEvent = async () => {
          let lvalue = await this.fetch()
            .then((x1) => {
              return x1.signalValue;
            })
            .catch((err) => o.writeDebug(`IO.Signal callback failed. >>> ${err.toString()}`, 3));

          for (let iii = 0; iii < callbacks.length; iii++) {
            try {
              callbacks[iii](lvalue);
            } catch (error) {
              o.writeDebug(`IO.Signal callback failed. >>> ${error.toString()}`, 3);
            }
          }
      ...
      this.searchSignals = (filter = {}) => {
        ...
        return o.Network.post('/rw/iosystem/signals/signal-search-ex', body, {
          ...
          .then((res) => {
            ...
            for (const item of obj._embedded.resources) {
              if (item._type === 'ios-signal-li') {
                ...
                signal.signalType = item.type;
                if (signal.signalType === 'AI' || signal.signalType === 'AO')
                  signal.signalValue = parseFloat(item.lvalue);
                else signal.signalValue = parseInt(item.lvalue);
                signal.category = item.category;
                signal.isSimulated = item.lstate === 'simulated';
                signal.quality = item.quality;
                signal.writeAccessLevel = item['write-access-level'];
      ...
      this.getDevice = function (networkName, deviceName) {
        ...
        let body = `network=${encodeURIComponent(networkName)}&name=${encodeURIComponent(
          deviceName
        )}`;
    ...
    o.CFG = new (function () {
        ...
        this.getInstances = function () {
          const getInstance = (item) => {
            ...
            for (var u = 0; u < item.attrib.length; u++) {
              ...
              if (
                typeof item.attrib[u]._title === 'string' &&
                item.attrib[u]._title.toLowerCase() === 'name'
              )
              ...
              for (const item of obj.state) {
                if (item._type === 'cfg-dt-instance') {
                  ...
                  for (var iii = 0; iii < item.attrib.length; iii++) {
                    ...
                    if (
                      typeof item.attrib[iii]._title === 'string' &&
                      item.attrib[iii]._title.toLowerCase() === 'name'
                    )
      ...
      this.verifyConfigurationFile = function (filePath, action = 'add') {
        if (isNonEmptyString(filePath) === false)
          return rejectWithStatus("Invalid parameter 'filePath'.");
        ...
        let body = `filepath=${encodeURIComponent(filePath)}&action-type=${encodeURIComponent(
          action
        )}`;
    ...
    o.Controller = new (function () {
      ...
      function Monitor(resource) {
        ...
        this.onchanged = function (newValue) {
          ...
          switch (resourceName) {
            case 'controller-state':
              if (newValue.hasOwnProperty('ctrlstate'))
                parsedValue = processString(newValue['ctrlstate']);
              break;
            case 'operation-mode':
              if (newValue.hasOwnProperty('opmode'))
                parsedValue = processString(newValue['opmode']);
              break;
            default:
              parsedValue = '';
          }
        ...
        const raiseEvent = async () => {
          const getValue = async () => {
            ...
            switch (resourceName) {
              case 'controller-state':
                if (state.hasOwnProperty('ctrlstate'))
                  parsedValue = processString(state['ctrlstate']);
                break;
              case 'operation-mode':
      ...
      this.setOperationMode = (mode) => {
        if (typeof mode !== 'string')
          return rejectWithStatus("Invalid parameter, 'mode' is not a string.");
      ...
      this.getEnvironmentVariable = (variable) => {
        ...
        return o.Network.get(`/ctrl/${encodeURIComponent(variable)}`)
          ...
          .catch((err) =>
            rejectWithStatus(`Could not get environment variable '${variable}'.`, err)
          );
      };
      ...
      this.verifyOption = (option) => {
        if (typeof option !== 'string' || option === '')
          return rejectWithStatus("Invalid parameter 'option'.");
      ...
      this.createBackup = (path, timeout = 60) => {
        ...
        if (typeof timeout !== 'number' || timeout <= 0)
          return rejectWithStatus('Invalid timeout.');
      ...
      this.restoreBackup = (
        ...
        if (
          ...
          return rejectWithStatus("Invalid parameter 'ignoreMismatches'.");
        if (typeof deleteDir !== 'boolean')
          return rejectWithStatus("Invalid parameter 'deleteDir'.");
        if (typeof includeSafetySettings !== 'boolean')
          return rejectWithStatus("Invalid parameter 'includeSafetySettings'.");
        if (include !== 'cfg' && include !== 'modules' && include !== 'all')
          return rejectWithStatus("Invalid parameter 'include'.");
            ...
            return o.Network.post('/ctrl/backup/restore', body)
              ...
              .catch((err) => {
                return o.Mastership.release()
                  ...
                  .catch((err) =>
                    Promise.reject(
                      'Could not start restore and failed to release mastership. >>> ' + err
                    )
                  );
              });
          })
      ...
      this.compress = (srcPath, destPath, timeout = 60) => {
        if (typeof srcPath !== 'string' || srcPath === '')
          return rejectWithStatus("Invalid 'srcPath'.");
        if (typeof destPath !== 'string' || destPath === '')
          return rejectWithStatus("Invalid 'destPath'.");
      ...
      this.decompress = (srcPath, destPath, timeout = 60) => {
        if (typeof srcPath !== 'string' || srcPath === '')
          return Promise.reject('Invalid srcPath.');
        if (typeof destPath !== 'string' || destPath === '')
          return Promise.reject('Invalid destPath.');
        if (isNaN(timeout) == true || timeout < 0) return Promise.reject('timeout not valid.');
      ...
      this.saveDiagnostics = (destPath, timeout = 60) => {
        ...
        return o.Network.post('/ctrl/diagnostics/save', body)
          .then((res) => {
            ...
            o.writeDebug(
              'saveDiagnostics: Failed to get the location of progress resource. The diagnostics will be saved but the call returns before it has completed.',
              2
            );
            return Promise.resolve();
          })
    ...
    o.FileSystem = new (function () {
      ...
      function Directory(path = '$HOME') {
        ...
        this.delete = function () {
          if (isDeleted === true) return rejectWithStatus('Directory has been deleted.');
          if (isNonEmptyString(dirPath) === false)
            return rejectWithStatus("Directory's path is not a valid string.");
        ...
        this.createFileObject = function (fileName) {
          if (isDeleted === true) return rejectWithStatus('Directory has been deleted.');
          if (isNonEmptyString(fileName) === false)
            return rejectWithStatus("New file's name is not a valid string.");
          if (isNonEmptyString(dirPath) === false)
            return rejectWithStatus("Directory's path is not a valid string.");
        ...
        this.copy = function (copyPath, overwrite, isRelativePath = true) {
          ...
          if (typeof overwrite !== 'boolean')
            return rejectWithStatus("Parameter 'overwrite' is not of valid type.");
      ...
      function File(path) {
        ...
        this.save = async function (overwrite, isBinary = false) {
          if (isDeleted === true) return rejectWithStatus('File has been deleted.');
          if (isNonEmptyString(filePath) === false)
            return rejectWithStatus("File's path is not a valid string.");
          if (typeof overwrite !== 'boolean')
            return rejectWithStatus("Parameter 'overwrite' is not of valid type.");
          if (typeof isBinary !== 'boolean')
            return rejectWithStatus("Parameter 'isBinary' is not of valid type.");
        ...
        this.copy = function (copyName, overwrite, isRelativePath = true) {
          if (isDeleted === true) return rejectWithStatus('File has been deleted.');
          if (isNonEmptyString(copyName) === false)
            return rejectWithStatus("New file's name is not a valid string.");
          if (typeof overwrite !== 'boolean')
            return rejectWithStatus("Parameter 'overwrite' is not of valid type.");
          if (typeof isRelativePath !== 'boolean')
    ...
    o.Elog = new (function () {
      ...
      function Event(number, language = 'en') {
        ...
        function fetch() {
          ...
          let url = `/rw/elog/seqnum/${encodeURIComponent(
            sequenceNumber
          )}?lang=${encodeURIComponent(languageId)}`;
      ...
      function Domain(number) {
         ...
         */
        this.clearElog = () => {
          if (typeof domainNumber !== 'number' || domainNumber < 0)
            return rejectWithStatus('Illegal domain number.');
        ...
        this.getEventsPaged = (language = 'en', count = 50, page = 1) => {
          ...
          let url = `/rw/elog/${encodeURIComponent(domainNumber)}?start=${encodeURIComponent(
            page
    ...
    o.UAS = new (function () {
        ...
        return o.Network.get('/users/login-info')
          .then((res) => {
            ...
            if (Object.keys(info).length !== 5)
              throw new Error('Could not get complete user info.');
      ...
      this.hasGrant = (grant) => {
        if (isNonEmptyString(grant) === false)
          return rejectWithStatus(
            'Failed to verify grant',
            "Inparameter 'grant' is not a valid string."
          );
        ...
        return o.Network.get(`/users/grant-exists?grant=${encodeURIComponent(grant)}`)
          ...
          .catch((err) => {
            if (err.hasOwnProperty('httpStatus') && err.httpStatus.code === 400) {
              if (
                err.hasOwnProperty('controllerStatus') &&
                err.controllerStatus.name === 'SYS_CTRL_E_INVALID_GRANT'
              ) {
                return Promise.resolve(false);
              }
            }
      ...
      this.hasRole = (role) => {
        if (isNonEmptyString(role) === false)
          return rejectWithStatus(
            ...
            "Inparameter 'role' is not a valid string."
          );

        return this.getUser()
          .then(() =>
            o.Network.get(`/uas/users/${encodeURIComponent(currentUserInfo.alias)}/roles`)
          )
          .then((res) => {
            let obj = parseJSON(res.responseText);
            if (typeof obj === 'undefined') return Promise.reject('Could not parse JSON.');
            ...
            for (const item of obj.state) {
              if (item._type === 'user-role') {
                if (item['rolename'].toLowerCase() === role.toLowerCase())
                  return Promise.resolve(true);
              }
            }
    ...
    o.Subscriptions = new (function () {
        ...
        if (
          ...
          for (let iii = 0; iii < newSubscriptions.length; iii++) {
            if (
              ...
              let title =
                newSubscriptions[iii].getTitle() !== undefined
                  ? newSubscriptions[iii].getTitle()
                  : '<Unknown>';

              o.writeDebug(
                `Subscribe on '${title}' rejected as subscription resource string not set.`,
                2
              );
              continue;
            }
            ...
            if (
              currentSubscriptions[resource] !== undefined &&
              currentSubscriptions[resource].length !== 0
            ) {
              ...
              body += `resources=${idx}&${idx}=${encodeURIComponent(
                resource
              )}&${idx}-p=${priority}`;
            }
          }
          ...
          for (let iii = 0; iii < newSubscriptions.length; iii++) {
            if (
              ...
              o.writeDebug(
                `Subscription on '${title}' rejected as subscription resource string not set.`,
                2
              );
              continue;
            }
          ...
          return o.Network.post('/subscription', body)
            .then((res) => {
              ...
              subscriptionGroup = websocketLocation.substring(
                websocketLocation.indexOf('poll/') + 5
              ); // ToDo Currently the only way to retrieve the group-id
      ...
      const processUnsubscribe = (removedSubscription) => {
        ...
        if (
          removedSubscription.getResourceString === undefined ||
          removedSubscription.getResourceString() === ''
        ) {
          let title =
            removedSubscription.getTitle() !== undefined
              ? removedSubscription.getTitle()
              : '<Unknown>';
          return rejectWithStatus(
            `Unsubscribe on '${title}' rejected as subscription resource not set.`,
            2
          );
        }
      ...
      this.unsubscribeToAll = () => {
        ...
        if (subscriptionGroup != null) {
          ...
          subscriptionGroup = null;
          currentSubscriptions = {};
          return o.Network.delete('/subscription/' + subscriptionGroup_temp)
            .then(() => Promise.resolve())
            .catch((err) => rejectWithStatus('Failed to unsubscribe to all.', err));
        } else {
      ...
      this.releaseAll = () => {
        try {
          ...
          if (typeof window.external !== 'undefined' && 'notify' in window.external) {
            ...
            mastershipCounter = 0;
            o.Network.send('POST', '/rw/mastership/edit/release', {
              'Content-Type': 'application/x-www-form-urlencoded;v=2.0',
            });
          }
    ...
    o.Network = new (function () {
      ...
      this.heartBeat = () => {
        this.get('/').then(
          ...
          (error) =>
            o.writeDebug(
              `Heartbeat Failed.  >>>  ${error.httpStatus.code} ${error.httpStatus.text}`,
              3
            )
        );
          ...
          req.onreadystatechange = () => {
            if (req.readyState === 4) {
              ...
              if (Math.floor(req.status / 100) !== 2) {
                  ...
                  return verfifyErrorCode(req.responseText)
                    .then((x) => {
                      ...
                      if (x.severity.toLowerCase() === 'error') {
                        o.writeDebug(
                          `RWS call '${call}', ${x.severity}: ${x.name}, '${x.description}'`,
                          1
                        );
                      }
                ...
                if (req.getResponseHeader('Content-Type') !== 'application/hal+json;v=2.0')
                  return resolve(req);
      ...
      this.get = (path, additionalRequestHeaders = {}) => {
        return this.send(
          'GET',
          path,
          Object.assign({ Accept: 'application/hal+json;v=2.0' }, additionalRequestHeaders)
        );
      };
      ...
      this.options = (path, additionalRequestHeaders = {}) => {
        return this.send(
          'OPTIONS',
          path,
          Object.assign({ Accept: 'application/xhtml+xml;v=2.0' }, additionalRequestHeaders)
        );
      };
