        ...
        */
        ...
        o.IO = new function () {
            ...
            function Signal(network, device, signal) {
                ...
                var isUnassigned = network === UNASSIGNED_TAG && device === UNASSIGNED_TAG;
                var signalPath = isUnassigned === true ? `${encodeURIComponent(signal)}` : `${encodeURIComponent(network)}/${encodeURIComponent(device)}/${encodeURIComponent(signal)}`;
                        ...
                        .then(res => {
                            ...
                            for (const item of obj._embedded.resources) {
                                if (item._type === "ios-signal-li") {
                                    ...
                                    if (signalType === 'AI' || signalType === 'AO') signalValue = parseFloat(item.lvalue);
                    ...
                    .then(res => {
                        ...
                        for (const item of obj._embedded.resources) {
                            if (item._type === "ios-signal-li") {
                                ...
                                if (signal.signalType === 'AI' || signal.signalType === 'AO') signal.signalValue = parseFloat(item.lvalue);
        ...
        o.UAS = new function () {
            ...
            this.hasRole = (role) => {
                ...
                return this.getUser()
                    .then(() => o.Network.get(`/uas/users/${encodeURIComponent(currentUserInfo.alias)}/roles`))
                    .then(res => {
                        let obj = parseJSON(res.responseText);
                        if (typeof obj === 'undefined') return Promise.reject('Could not parse JSON.');
            ...
            this.releaseAll = () => {
                try {
                    ...
                    if (typeof (window.external) !== 'undefined' && ('notify' in window.external)) {
                        ...
                        mastershipCounter = 0;
                        o.Network.send("POST", "/rw/mastership/edit/release", { "Content-Type": "application/x-www-form-urlencoded;v=2.0" });
                    }
