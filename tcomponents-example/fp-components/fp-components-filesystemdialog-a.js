...
(function (o) {
    ...
    if (!o.hasOwnProperty("Filesystemdialog_A")) {
                ...
                return new Promise(async (resolve,reject)=>{
                    ...
                    async function updateCurrentFolder(newFolderPath = null) {
                        ...
                        if(newFolderPath !== null) {
                            ...
                            try {
                                ...
                                while(
                                    ...
                                    (newFolderPath.charAt(newFolderPath.length-1) === "\\" || newFolderPath.charAt(newFolderPath.length-1) === "/") ) 
                                {
                        ...
                        try {
                            ...
                            if(!newFolderPath.toLowerCase().includes(`${FPComponents.Filesystemdialog_A.ROOT_FOLDER_HOME}`.toLowerCase()) &&
                                !newFolderPath.toLowerCase().includes(`${FPComponents.Filesystemdialog_A.ROOT_FOLDER_TEMP}`.toLowerCase()) &&
                                !newFolderPath.toLowerCase().includes(`${FPComponents.Filesystemdialog_A.ROOT_FOLDER_BACKUP}`.toLowerCase()))
                            {
                    ...
                    async function displayDirectoryContent(directoryContent) {
                        ...
                        if(fileSystemObjectType === `${FPComponents.Filesystemdialog_A.FileSystemObjectType.File}`) {
                            ...
                            dirArr.forEach(dir => {
                                fileSysObjArr.push({type: `${FPComponents.Filesystemdialog_A.FileSystemObjectType.Folder}`, content: dir});
                            });
        
                            if(validFileEndings !== null && validFileEndings.length > 0) {
                                
                                // Only display valid file endings
                                    ...
                                    validFileEndings.forEach(fileEnding => {
                                        ...
                                        if(fileEnding.trim().length > 0) {
                                            ...
                                            if(file.name.toLowerCase().endsWith(fileEnding.toLowerCase())) {
                                                fileSysObjArr.push({type: `${FPComponents.Filesystemdialog_A.FileSystemObjectType.File}`, content: file});
                                            }    
                                        }
                                    });
                                }); 
                        ...
                        // Select rows in content directory, if selection contains object name
                    ...
                    function sortFolderContent(selectedIndex, selectedText) {
                        ...
                        // Same dropDown index selected twice? Swap sort order
        ...
        o.Filesystemdialog_A.VERSION = "1.4";
    }
