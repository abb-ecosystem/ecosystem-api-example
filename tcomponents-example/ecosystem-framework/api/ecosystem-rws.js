  ...
  rws.RWS = new (function () {
    ...
    const requestMastership = function (type = "edit") {
      if (type === "edit")
        return RWS.Mastership.request()
          .then(() => Promise.resolve())
          .catch((err) =>
            API.rejectWithStatus("Could not get Mastership.", err)
          );
    ...
    this.MOTIONSYSTEM = new (function () {
      ...
      this.getMechunit = async function (name = "ROB_1") {
        try {
          let res = await RWS.Network.get(
            `/rw/motionsystem/mechunits/${name}?continue-on-err=1`
          );
          let obj = parseJSON(res.responseText);
      ...
      this.setMechunit = async function ({
        ...
        body += totalPayload
          ? (body ? "&" : "") + "total-payload=" + totalPayload
          : "";
    ...
    this.RAPID = new (function () {
       ...
       * @returns {Promise<any>}
       */
      this.loadModule = async function (
        path,
        replace = false,
        taskName = "T_ROB1"
      ) {
      ...
      this.movePPToCursor = function (moduleName, taskName, line, column) {
        ...
        let body = `module=${encodeURIComponent(
          moduleName
        )}&line=${encodeURIComponent(line)}&column=${encodeURIComponent(
          column
        )}`;
